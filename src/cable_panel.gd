...
func _fill_hand() -> void:
	while current_hand.size() < HAND_SIZE:
		...
		_spawn_random_piece()
	_reposition_hand()
	print("HAND FILLED - count: ", current_hand.size())
	for i in range(current_hand.size()):
		var p = current_hand[i]
		print("  [", i, "] type=", p.piece_type, " sprite_null=", p.sprite == null, " texture_null=", (p.sprite.texture == null if p.sprite else "N/A"), " visible=", p.visible)
...
func _spawn_random_piece() -> void:
	...
	add_child(piece)
	print("SPAWN: type=", chosen_type, " sprite_null_right_after_add_child=", piece.sprite == null)

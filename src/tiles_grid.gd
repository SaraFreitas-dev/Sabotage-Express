...
func check_win() -> bool:
	...
	# Fully explore the whole connected network from the entry first.
	# We do NOT stop early when the exit is found, because pieces
	# discovered later (e.g. a tip capping a T-shape's branch) might not
	# have been expanded yet, which would cause a false "open pipe" result.
	...
	if not visited.has(exit_neighbor):
		print("FAILED: did not reach the exit")
		return false

	var exit_piece: Node = occupied_cells[exit_neighbor]
	var exit_connections: Array = _get_piece_connections(exit_piece)
	if not exit_connections.has("right"):
		print("FAILED: piece before exit does not connect 'right'")
		return false

	# Path reaches the exit; now that the whole network has been fully
	# explored, confirm there are no dangling/open connections in it.
